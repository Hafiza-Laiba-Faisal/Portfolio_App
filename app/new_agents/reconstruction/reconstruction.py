...
reconstruction_tool_instance = ReconstructionTool()

# @function_tool
# async def reconstruction_tool_fn(input_data: dict) -> ReconstructionOutput:
#     """
#     Tool for Reconstruction Operations:
#     - Assess damage
#     - Recommend actions
#     - Track rehab progress
#     - Generate coordination notes
#     """
#     result = await reconstruction_tool_instance.run(input_data)
#     return result  # Return Pydantic model directly

async def reconstruction_tool_fn(input_data: dict) -> dict:
    """
    Tool for Reconstruction Operations:
    - Assess damage
    - Recommend actions
    ...
    """
    result = await reconstruction_tool_instance.run(input_data)
    return result.model_dump()  # convert Pydantic -> dict
# -------------------------------
# Reconstruction Agent
# -------------------------------
reconstruction_agent = Agent(
    name="ReconstructionAgent",
    instructions="Manage post-disaster reconstruction: assess damage, allocate aid, track rehab, and provide recommendations.",
    tools=[reconstruction_tool_fn],
    model=model  # Use the Gemini LLM client already defined
)
...
if __name__ == "__main__":
    ...
    async def test():
        result = await reconstruction_tool_fn(sample_input)
        # result is already a dict, no need for model_dump()
